...
*/
public static function getListenerConfig($listener, $target, $refId = null)
{
	$default = array(
		'on' => null,
		'callback' => null,
	...
	if ($listener instanceof ListenerConfigInterface) {
		$listener = $listener->getListenerConfig();
...
public static function addListenerToRuntime(<?php echo $listenerName; ?> $l) {
	...
	if (!in_array($l, self::$globalListeners[$l->getTarget()][$l->getEvent()])) { 	
		self::$globalListeners[$l->getTarget()][$l->getEvent()][$l->getId()] = $l;
	}
}
...
public static function removeListenerFromRuntime(<?php echo $listenerName; ?> $l) {
	...
	unset(self::$globalListeners[$l->getTarget()][$l->getEvent()][$l->getId()]);
}
